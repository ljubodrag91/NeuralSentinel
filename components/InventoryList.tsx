...
const InventoryList: React.FC<InventoryListProps> = ({ panelSlots, globalLowSlot }) => {
  ...
  const renderCard = (item: any, type: 'module' | 'consumable') => {
    ...
    const borderColor = isSelected ? accentColor : '#27272a';
          ...
          <div className="flex justify-between items-start relative z-10">
            <div className="flex items-center gap-3">
              ...
              <div className="flex flex-col">
                <span className="text-[10px] font-black uppercase tracking-widest text-zinc-200 group-hover:text-white transition-colors truncate max-w-[120px]">{item.name}</span>
                <span className="text-[7px] font-mono text-zinc-600 uppercase">{type === 'module' ? `ID: ${item.id}` : `TYPE: ${item.type}`}</span>
              </div>
            </div>
            {isAssigned && <div className="flex items-center gap-1 px-1.5 py-0.5 bg-zinc-900 border border-zinc-800"><div className="w-1 h-1 bg-green-500 rounded-full animate-pulse"></div><span className="text-[7px] font-black text-zinc-500 uppercase tracking-wider">EQUIPPED</span></div>}
          </div>
          <div className="grid grid-cols-2 gap-2 relative z-10 mt-1">
             <div className="bg-zinc-950/50 p-2 border border-zinc-900 flex flex-col"><span className="text-[7px] text-zinc-600 font-black uppercase">Capacity</span><span className="text-[9px] font-mono text-zinc-300">{type === 'module' ? `${item.maxCharges} CHG` : (item.unlimited ? '∞' : `x${item.count}`)}</span></div>
             <div className="bg-zinc-950/50 p-2 border border-zinc-900 flex flex-col"><span className="text-[7px] text-zinc-600 font-black uppercase">{type === 'module' ? 'Rate' : 'Cost'}</span><span className="text-[9px] font-mono text-zinc-300">{type === 'module' ? `${item.rechargeRate}s` : `${item.cost} CHG`}</span></div>
          </div>
          {type === 'consumable' && item.features && <div className="flex flex-wrap gap-1 mt-1 relative z-10">{item.features.map((f: string) => <span key={f} className="text-[6px] font-black bg-zinc-900 text-zinc-500 px-1 py-0.5 border border-zinc-800 uppercase tracking-wider">{f}</span>)}</div>}
          {isSelected && <div className="absolute bottom-0 left-0 w-full h-0.5" style={{ backgroundColor: accentColor }}></div>}
        </div>
      </Tooltip>
    );
  };
  ...
  return (
    <div className="flex flex-col h-full overflow-hidden">
      <div className="flex border-b border-zinc-900 mb-6 shrink-0">
        {(['CONSUMABLES', 'MODULES', 'OTHER'] as InventoryTab[]).map(tab => (
          <button key={tab} onClick={() => { setActiveTab(tab); setSelectedItem(null); }} className={`flex-1 py-3 text-[10px] font-black uppercase tracking-[0.2em] transition-all relative ${activeTab === tab ? 'text-teal-400 bg-teal-950/10' : 'text-zinc-600 hover:text-zinc-300 hover:bg-zinc-900/40'}`}>{tab}{activeTab === tab && <div className="absolute bottom-0 left-0 w-full h-[2px] bg-teal-500 shadow-[0_0_10px_#00ffd5]"></div>}</button>
        ))}
      </div>
