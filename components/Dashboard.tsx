...
const Dashboard: React.FC<DashboardProps> = ({ 
  ...
  const mainProbeLauncherId = settings.panelSlots['GLOBAL_SYSTEM_PROBE']?.probeSlot?.launcherId;
  const isMainProbeEquipped = !!mainProbeLauncherId;
  
  const charges = mainProbeLauncherId ? serverService.getCharges(mainProbeLauncherId) : 0;
  const cooldown = mainProbeLauncherId ? serverService.getCooldown(mainProbeLauncherId) : 0;
  const hasBooster = launcherSystem.isBoosterActive();
  
  const isAvailable = isConnected && isMainProbeEquipped && (charges > 0 || hasBooster) && cooldown === 0;
  
  const getMainProbeStatusText = () => {
    if (!isMainProbeEquipped) return "Disabled - No Module Assigned";
    if (!isConnected) return "Disabled - System Node Offline";
    if (cooldown > 0 && !hasBooster) return `System Reloading... (${(cooldown/1000).toFixed(1)}s)`;
    if (charges === 0 && !hasBooster) return "Disabled - Out of Core Charges";
    return "Ready - Historical Core Analysis Available";
  };
              ...
              <div className="flex items-center justify-between gap-8 flex-1 min-h-0 px-4">
                ...
                <div className="flex flex-col items-center justify-center relative group w-64 shrink-0">
                  ...
                  <div className="relative z-10">
                    <Tooltip name="HISTORICAL_CORE_PROBE" source="NEURAL_NETWORK" desc={`${getMainProbeStatusText()}. Consumption: 1 Core Charge. Limits: 4000 Tokens.`} variant="purple">
                      <button 
                      ...
                      >
                        ...
                        {(!isMainProbeEquipped) && (
                           <div className="absolute inset-0 bg-black/80 flex items-center justify-center z-20">
                             <span className="text-[8px] font-black text-red-500 tracking-[0.2em]">NO_MODULE</span>
                           </div>
                        )}
                        {(isMainProbeEquipped && charges === 0 && !hasBooster && cooldown === 0) && (
                           <div className="absolute inset-0 bg-red-950/20 flex items-center justify-center z-20 backdrop-blur-[1px]">
                             <span className="text-[8px] font-black text-red-500 tracking-[0.2em]">DEPLETED</span>
                           </div>
                        )}
                      </button>
                    </Tooltip>
                  ...
                  <span className={`absolute bottom-[-30px] text-[7px] font-black uppercase tracking-[0.4em] pointer-events-none whitespace-nowrap transition-colors ${isProbeActive ? 'text-purple-500' : 'text-zinc-800'}`}>
                    {isProbeActive ? 'PROBE_SEQUENCE_ACTIVE' : 'Neural_Bridge_Standby_Ready'}
                  </span>
                </div>
